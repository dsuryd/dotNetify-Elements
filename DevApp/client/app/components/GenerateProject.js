...
export const GenerateProject = ({ caption, title, sourceUrl, sourceDir, useAnchor }) => {
  ...
  const generate = async () => {
    const data = {
      ...
      templateType: "dotnet",
      tags: { spa_template: projectName || "MyProject" }
    };
    ...
    if (!response.ok) {
      ...
      alert(
        "Oops! This is unexpected, the generator appears to be off-line.  You'll have to manually download the template from https://github.com/dsuryd/dotNetify-elements.  Sorry about that!"
      );
      return;
    }
  ...
  return (
    <Fragment>
      {useAnchor ? (
        <Anchor onClick={() => setShow(true)}>{caption}</Anchor>
      ) : (
