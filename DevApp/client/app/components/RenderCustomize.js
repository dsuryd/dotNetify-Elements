...
\`\`\``;
  ...
  getComponentProps = selected => {
    let componentProps = Object.keys(this.componentTypes).reduce(
      (all, item) => ({ ...all, [item]: this.componentTypes[item] }),
      {}
    );
  ...
  render() {
    ...
    const propsText = selected
      ? `${utils.toCamelCase(selected)}=withHighlight(${this.props.name}.componentTypes.${selected})`
      : "";
    ...
    const content = (
      <Panel>
        ...
        <Markdown>{this.buildCode(propsText)}</Markdown>
      </Panel>
    );
